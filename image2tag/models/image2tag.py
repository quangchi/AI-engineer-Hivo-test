...
class Image2tag(nn.Module):
    ...
    def generate_tag(self,
        ...
        tag_output = []
        for b in range(bs):
            index = np.argwhere(tag[b] > threshold)
            indexs = np.argwhere(tag[b] > threshold).flatten()
            scores = tag[b][indexs]
            
            token = self.tag_list[index].squeeze(axis=1)
            tag_output.append(token)

        return tag_output[0], scores
...
def image2tag(pretrained='', **kwargs):
    model = Image2tag(**kwargs)
    if pretrained:
